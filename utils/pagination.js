 ...
 * @param {Number} [options.totalpages=true]
...
function paginate(query, options, callback) {
  ...
  let limit = parseInt(options.limit) || Number.MAX_SAFE_INTEGER;
  ...
  if (limit) {
    ...
    if (Object.keys(query).length > 0) {
      tmpCount = this.countDocuments(query).exec();
    } else {
      tmpCount = this.estimatedDocumentCount().exec();
    }
    promises = {
      docs: docsQuery.exec(),
      count: tmpCount
